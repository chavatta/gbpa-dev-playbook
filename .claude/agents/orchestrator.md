...
9. **Gate de skills:** se algum ponteiro trouxe `skill_candidates`, consolide-os no backlog do projeto, `docs/skill-backlog.md` (arquivo de escrita exclusiva sua; crie a partir de `multi-agents/templates/SKILL-BACKLOG.template.md` se não existir). Candidate novo vira linha; candidate que já está no backlog incrementa o contador de ocorrências. Rejeite candidate sem evidência com fonte (padrão declarado em doc, ou tasks onde se repetiu). Avalie pela regra dos 3 (`multi-agents/SKILLS-GOVERNANCE.md`): aprovado → status `aprovada` e delegue a autoria ao `documenter` (via `skill-creator`), nascendo no projeto; não aprovado → mantenha como `candidate` com a decisão anotada e registre no `run-log.md` — a evidência acumulada decide a reavaliação futura.
