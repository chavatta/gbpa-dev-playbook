...
- **Os dois falsos positivos do `block-dangerous-git.mjs` reportados em 2026-09-15.** O diagnóstico registrado aqui estava incompleto: não era só quebra de linha. A causa-raiz é que o token de argumento (`[^ ]+`) atravessava separador de comando — o push de uma branch seguido de `gh pr create --base main` com `&&`, numa linha só, também bloqueava, e o mesmo defeito fazia `git push origin x ; rm -f tmp` virar "force push". O heredoc era um segundo defeito: o corpo era lido como comando mesmo quando é só dado (`git commit -F -`, `cat > doc.md`). A auditoria de 2026-09-23 achou mais dois falsos positivos (verbo e caminho protegido casados em comandos diferentes — `cat .claude/settings.json ; rm tmp`; e `ln=5` lido como `ln`) e três bypasses no hook em produção (`git -C repo push --force`, `cd .claude/hooks && cp …`, `cd .claude && cp … settings.json`). Todos viraram payload; a proposta passa 97/97. Nesta mesma sessão o hook em produção bloqueou duas vezes a edição deste arquivo — é o defeito, reproduzido ao vivo.
