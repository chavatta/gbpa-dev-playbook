...
- **Os dois falsos positivos do `block-dangerous-git.mjs` reportados em 2026-09-15.** O diagnóstico registrado aqui estava incompleto: não era só quebra de linha. A causa-raiz é que o token de argumento (`[^ ]+`) atravessava separador de comando — o push de uma branch seguido de `gh pr create --base main` com `&&`, numa linha só, também bloqueava, e o mesmo defeito fazia `git push origin x ; rm -f tmp` virar "force push". O heredoc era um segundo defeito: o corpo era lido como comando mesmo quando é só dado (`git commit -F -`, `cat > doc.md`). A auditoria de 2026-09-23 achou mais dois falsos positivos (verbo e caminho protegido casados em comandos diferentes — `cat .claude/settings.json ; rm tmp`; e `ln=5` lido como `ln`) e onze bypasses no hook em produção (entre eles `git -C repo push --force`, `cd .claude/hooks && cp …`, herestring para interpretador, `rsync` e `git checkout <ref> -- .claude/settings.json`). Todos viraram payload; a proposta passa 145/145, e os dois gates desta task reprovaram as duas primeiras versões por regressões que hoje estão no banco. Nesta mesma sessão o hook em produção bloqueou duas vezes a edição deste arquivo — é o defeito, reproduzido ao vivo.
...
| 2 | **Alias `model: fable`** | ~~Válido no Claude Code atual; a auto-verificação de modelo nos agentes cobre o fallback silencioso. Não precisa trocar pelo id completo~~ — **superado em 2026-09-23:** todos os agentes passaram a Opus 5.5 (`claude-opus-5-5`), exceto o Security-SRE, que ficou em Fable 5.1 (`claude-fable-5-1`), e o Documenter, que segue em Haiku 4.5 (alias `haiku`). Opus e Fable ficam fixados pelo ID completo, porque o alias trocaria a versão de um gate sem revisão do ADR | ADR-001 (revisão trimestral) |
...
| 4 | **Nome dos agentes** | Mantido o sufixo de modelo (`reviewer-fable`; hoje `reviewer-opus`, e `security-sre-fable` segue igual), com nome-base nas referências de processo. Requisito: o modelo em execução tem de estar claro — garantido por frontmatter + auto-verificação + campo `model` obrigatório no ponteiro de handoff | `ADR-001` → "Nomenclatura dos agentes e visibilidade do modelo" |
