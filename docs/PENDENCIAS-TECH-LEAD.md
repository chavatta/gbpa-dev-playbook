...
| 2 | **Alias `model: fable`** | ~~Válido no Claude Code atual; a auto-verificação de modelo nos agentes cobre o fallback silencioso. Não precisa trocar pelo id completo~~ — **superado em 2026-09-23:** Orchestrator, Architect e Reviewer passaram a Opus 5.5 (`claude-opus-5-5`) e o Security-SRE ficou em Fable 5.1 (`claude-fable-5-1`) — todos pelo ID completo, porque o alias trocaria a versão de um gate sem revisão do ADR | ADR-001 (revisão trimestral) |
