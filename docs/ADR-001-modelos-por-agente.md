...
`orchestrator`, `architect` e `reviewer` passam de **Fable 5** para **Opus 5.5**. O `security-sre` (ADR-002) **fica em Fable**, agora fixado em **Fable 5.1**. *Primeira etapa:* o racional de *onde* fica o modelo de topo não muda — decomposição, design e os dois gates —, muda *qual* modelo ocupa esses nós. A segunda etapa, mais abaixo, leva o topo também aos executores — e os dois gates passam a rodar em famílias diferentes: em task sensível, a lente de correção (Reviewer, Opus) e a de segurança (Security-SRE, Fable) não compartilham os pontos cegos de um mesmo modelo.
